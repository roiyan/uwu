...
// ---------- Operator link + PIN ----------
//
// Authenticated routes (couple) generate / reset the link. Public
// routes (the operator on the venue floor) verify the PIN and the
// resulting session key on every page load. The session key is
// `${token}:${pin}` stored in the operator's localStorage — both
// halves are revalidated server-side, so a reset from the dashboard
// invalidates any in-flight operator sessions on next reload.
...
export async function generateOperatorLinkAction(
  eventId: string,
): Promise<ActionResult<OperatorLinkResult>> {
  return rotateOperatorLink(eventId);
}
...
async function rotateOperatorLink(
  ...
  const { generateOperatorPin, generateOperatorToken, buildOperatorUrl, appBaseUrl } =
    await import("@/lib/utils/operator");
  ...
  const result = await withAuth(eventId, "editor", async () => {
    ...
    await db
      ...
      .set({
        operatorPin: pin,
        operatorToken: token,
        operatorTokenCreatedAt: createdAt,
        updatedAt: createdAt,
      })
...
// Per-IP-ish PIN attempt limiter. In-memory module state — fine for
// the operator-link use case (one event, low fan-in). A few wrong
// guesses in a minute push the next attempt out by 60 seconds, which
// is enough to make sequential brute force untenable in practice
// without locking out the legitimate operator after a typo.
const PIN_ATTEMPT_WINDOW_MS = 60_000;
const PIN_ATTEMPT_LIMIT = 5;
const pinAttempts = new Map<string, number[]>();

function recordAttempt(eventId: string): boolean {
  const now = Date.now();
  const recent = (pinAttempts.get(eventId) ?? []).filter(
    (ts) => now - ts < PIN_ATTEMPT_WINDOW_MS,
  );
  if (recent.length >= PIN_ATTEMPT_LIMIT) {
    pinAttempts.set(eventId, recent);
    return false;
  }
  recent.push(now);
  pinAttempts.set(eventId, recent);
...
function clearAttempts(eventId: string) {
  pinAttempts.delete(eventId);
}

export type OperatorVerifyResult = { sessionKey: string };

export async function verifyOperatorPinAction(
  eventId: string,
  token: string,
  pin: string,
): Promise<ActionResult<OperatorVerifyResult>> {
  if (!eventId || !token || !pin || pin.length !== 4 || !/^\d{4}$/.test(pin)) {
    return { ok: false, error: "PIN harus 4 digit angka." };
  }
  if (!recordAttempt(eventId)) {
    return {
      ok: false,
  ...
  if (!event.checkinEnabled)
    return { ok: false, error: "Check-in belum diaktifkan." };
  if (!event.operatorToken || event.operatorToken !== token)
    return { ok: false, error: "Link tidak valid." };
  if (!event.operatorPin || event.operatorPin !== pin)
    return { ok: false, error: "PIN salah." };

  clearAttempts(eventId);
  return { ok: true, data: { sessionKey: `${token}:${pin}` } };
}
...
export async function verifyOperatorSessionAction(
  ...
  const [token, pin] = sessionKey.split(":");
  if (!token || !pin)
  ...
  if (
    ...
    event.operatorToken !== token ||
    event.operatorPin !== pin
  ) {
    return { ok: false, error: "Sesi sudah tidak berlaku." };
  }
